import { UNFOLLOW_USER } from "@/const/serverPath";
...
const useUnFollowMutation = () => {
  ...
  return useMutation({
    ...
    /**
     * 언팔로우시 결과에 관계없이 선제적으로 해당유저의 팔로워를 변화시키고, 팔로잉 여부를 True로 설정
     * @param userNo 언팔로우할 유저 PK
     */
...
export const followUserMutationFn = async (userNo: string) => {
  const token = getTokenFromLocalStorage();
  const { data } = await axios.post<{ userFollowNo: number }>(
    UNFOLLOW_USER(userNo),
    null,
    {
